...
function bindNavigation() {
  ...
  navItems.forEach(item => {
    item.addEventListener('click', () => {
      ...
      if (summaryCard) {
        summaryCard.style.display = (targetTab === 'dashboard') ? 'block' : 'none';
      }

      // ⭐ 탭 이동 시 각 화면의 데이터를 즉시 최신 상태로 재렌더링!
  ...
  const btnAddTxDirect = document.getElementById('btn-add-tx-direct');
  if (btnAddTxDirect) {
    btnAddTxDirect.addEventListener('click', () => {
      openTradeModal();
    });
  }
}
...
window.deleteTxDirect = function(txId) {
  if (confirm('이 매매일지 거래 기록을 삭제하시겠습니까?')) {
    appStore.data.transactions = (appStore.data.transactions || []).filter(t => t.id !== txId);
    appStore.saveData();
    renderTransactions();
    renderDashboard();
  }
...
window.openEditStrategyModal = function(stockId) {
  const stock = appStore.getStocks().find(s => s.id === stockId);
  if (!stock) return;

  const newCycle = prompt(`[${stock.name}] 리밸런싱 주차 주기를 입력하세요 (1, 2, 3, 4주차):`, stock.cycleWeeks || 2);
  if (newCycle === null) return;

  const newBand = prompt(`[${stock.name}] VR 밴드 비율 (%)을 입력하세요:`, stock.vrBandPercent || 15);
  if (newBand === null) return;

  const newG = prompt(`[${stock.name}] 적립 지수 (G)를 입력하세요:`, stock.G || 15);
  if (newG === null) return;

  const newStep = prompt(`[${stock.name}] 수량 주문단위 (주)를 입력하세요:`, stock.qtyStep || 10);
  if (newStep === null) return;

  const newBuy = prompt(`[${stock.name}] 목표 매수가를 입력하세요 (${stock.currency || 'KRW'}):`, stock.targetBuyPrice || 0);
  if (newBuy === null) return;

  const newSell = prompt(`[${stock.name}] 목표 매도가를 입력하세요 (${stock.currency || 'KRW'}):`, stock.targetSellPrice || 0);
  if (newSell === null) return;

  stock.cycleWeeks = parseInt(newCycle) || stock.cycleWeeks;
  stock.vrBandPercent = parseFloat(newBand) || stock.vrBandPercent;
  stock.G = parseFloat(newG) || stock.G;
  stock.qtyStep = parseInt(newStep) || stock.qtyStep;
  stock.targetBuyPrice = parseFloat(newBuy) || stock.targetBuyPrice;
  stock.targetSellPrice = parseFloat(newSell) || stock.targetSellPrice;

  appStore.saveData();
  alert(`✅ [${stock.name}] 매매 전략 파라미터가 성공적으로 반영되었습니다.`);
  renderStrategies();
  renderDashboard();
};
...
function renderTransactions(filterStockId = 'ALL') {
  ...
  // 종목 드롭다운 필터 옵션 채우기 (동적 갱신)
  if (filterSelect) {
    const currentVal = filterSelect.value || filterStockId;
    filterSelect.innerHTML = '<option value="ALL">전체 종목 보기</option>';
    stocks.forEach(stk => {
      const opt = document.createElement('option');
      opt.value = stk.id;
      opt.textContent = `${stk.name} (${stk.code})`;
      if (stk.id === currentVal) opt.selected = true;
  ...
  if (filteredTxs.length === 0) {
    txContainer.innerHTML = `<div style="text-align:center; padding:30px; color:var(--text-muted); font-size:0.85rem;">
      ${filterStockId === 'ALL' ? '기록된 거래 내역이 없습니다. 상단 "+ 거래 기록 추가" 버튼으로 입력해보세요.' : '선택한 종목의 매매일지 거래 내역이 없습니다.'}
    </div>`;
    return;
  }
  ...
  filteredTxs.forEach(tx => {
    ...
    item.innerHTML = `
      ...
      <div style="display:flex; align-items:center; gap:12px;">
        ...
        <button class="icon-btn" onclick="deleteTxDirect('${tx.id}')" title="거래 내역 삭제" style="color:#f87171; padding:4px 6px; border:none; background:transparent; cursor:pointer;">🗑️</button>
      </div>
    `;
  ...
  stocks.forEach(stock => {
    ...
    card.innerHTML = `
      <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px;">
        <div>
          <h4 style="font-size:1.05rem; font-weight:700;">${stock.name} (${stock.code})</h4>
          <span style="font-size:0.75rem; color:var(--text-sub);">통화: ${stock.currency || 'KRW'} | 보유: ${(stock.holdings || 0).toLocaleString()}주</span>
        </div>
        <span class="exchange-rate-badge">${stock.strategyType || 'VR'}</span>
      </div>

      <div style="display:grid; grid-template-columns:repeat(2, 1fr); gap:10px; font-size:0.85rem; background:rgba(15,23,42,0.6); padding:12px; border-radius:12px; border:1px solid var(--border-color); margin-bottom:12px;">
        <div>리밸런싱 주기: <strong style="color:#a5b4fc;">${stock.cycleWeeks || 2}주차</strong></div>
        <div>VR 밴드 비율: <strong style="color:#a5b4fc;">±${stock.vrBandPercent || 15}%</strong></div>
        <div>적립 지수(G): <strong style="color:#a5b4fc;">${stock.G || 15}</strong></div>
        <div>수량 주문단위: <strong style="color:#a5b4fc;">${stock.qtyStep || 10}주</strong></div>
        <div>목표 매수가: <strong style="color:#34d399;">${formatCurrency(stock.targetBuyPrice || 0, stock.currency)}</strong></div>
        <div>목표 매도가: <strong style="color:#f87171;">${formatCurrency(stock.targetSellPrice || 0, stock.currency)}</strong></div>
      </div>

      <div style="display:flex; justify-content:flex-end;">
        <button class="btn-primary" onclick="openEditStrategyModal('${stock.id}')" style="padding:6px 14px; font-size:0.78rem; font-weight:700;">
          ⚙️ 전략 파라미터 수정
        </button>
      </div>
    `;
