...
function renderDashboard() {
  ...
  stocks.forEach(stock => {
    ...
    const invCurrency = stock.holdings * stock.avgPrice;
    const evalCurrency = stock.holdings * stock.currentPrice;
    ...
    card.innerHTML = `
      <div class="stock-card-top">
        <div class="stock-name-group">
          ...
          <span class="stock-code">${stock.code} · ${stock.currency}</span>
        </div>
        <div class="stock-badge ${pnlPercent >= 0 ? 'badge-up' : 'badge-down'}">
      ...
      <div class="stock-card-details">
        <div class="stock-detail-item">
          ...
          <span class="stock-detail-val">${formatCurrency(stock.currentPrice, stock.currency)}</span>
        </div>
        <div class="stock-detail-item">
        ...
        <div class="stock-detail-item">
          ...
          <span class="stock-detail-val">${stock.holdings.toLocaleString()}주</span>
        </div>
      </div>
  ...
  document.getElementById('header-exchange-rate').textContent = `실시간 환율: $1 = ₩${currentExchangeRate.toLocaleString()}`;
  document.getElementById('total-assets-val').textContent = `₩${Math.round(totalEvaluationKRW).toLocaleString()}`;
...
function openStockDetailModal(stock) {
  ...
  const basePrice = lastRow ? lastRow.price : (stock.currentPrice || stock.avgPrice);
  const baseQty = lastRow ? lastRow.qty : stock.holdings;
...
function renderTransactions() {
  ...
  if (txs.length === 0) {
    txContainer.innerHTML = '<div style="text-align:center; padding:30px; color:var(--text-muted);">기록된 거래 내역이 없습니다.</div>';
    return;
  }
  ...
  txs.forEach(tx => {
    ...
    item.innerHTML = `
      ...
      <div style="text-align:right;">
        <div style="font-size:0.95rem; font-weight:600;">${tx.quantity}주 @ ${formatCurrency(tx.price, stock.currency)}</div>
        <div style="font-size:0.75rem; color:var(--text-muted);">수수료: ${formatCurrency(tx.fee, stock.currency)}</div>
      </div>
    `;
